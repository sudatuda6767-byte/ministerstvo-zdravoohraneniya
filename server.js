...
async function qOne(text, params = []) { return (await q(text, params))[0] || null; }

async function initDB() {
  await q(`CREATE TABLE IF NOT EXISTS users (
    id SERIAL PRIMARY KEY, login TEXT UNIQUE NOT NULL, password TEXT NOT NULL,
    nickname TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'Сотрудник', created_at TEXT NOT NULL
  )`);
  await q(`CREATE TABLE IF NOT EXISTS blacklist (
    id SERIAL PRIMARY KEY, fio TEXT NOT NULL, fio_norm TEXT NOT NULL, date_added TEXT NOT NULL,
    date_until TEXT, reason TEXT NOT NULL, added_by_role TEXT NOT NULL, added_by_fio TEXT NOT NULL,
    added_by_user_id INTEGER, removal_fee TEXT, removal_action TEXT,
    is_removed INTEGER DEFAULT 0, removed_at TEXT, removed_by_role TEXT, removed_by_fio TEXT, removed_by_user_id INTEGER,
    is_amnestied INTEGER DEFAULT 0, amnestied_at TEXT, amnestied_by_role TEXT, amnestied_by_fio TEXT, amnestied_by_user_id INTEGER
  )`);
  await q(`CREATE TABLE IF NOT EXISTS applications (
    id SERIAL PRIMARY KEY, case_number TEXT NOT NULL, fio TEXT NOT NULL, fio_norm TEXT NOT NULL,
    phone TEXT, special_comm TEXT, special_id TEXT, interview_datetime TEXT,
    interview_responsible TEXT, acceptance_responsible TEXT,
    passport_url TEXT, license_url TEXT, medcard_url TEXT,
    submitted_by_role TEXT, submitted_by_fio TEXT, submitted_by_user_id INTEGER,
    status TEXT DEFAULT 'pending', reject_reason TEXT,
    decided_by_role TEXT, decided_by_fio TEXT, decided_at TEXT,
    discord_channel_id TEXT, discord_message_id TEXT, created_at TEXT NOT NULL
  )`);
  await q(`CREATE TABLE IF NOT EXISTS files (
    id TEXT PRIMARY KEY, mime TEXT NOT NULL, data BYTEA NOT NULL, created_at TEXT NOT NULL
  )`);
...
function hasRole(member, allowed) {
  ...
  return [...member.roles.cache.keys()].some(id => allowed.includes(id));
}
  ...
  try {
    if (interaction.isChatInputCommand()) {
      ...
      if (cmd === 'bl-add') {
        ...
        const fio=interaction.options.getString('fio',true).trim(), reason=interaction.options.getString('reason',true).trim();
        const until=(interaction.options.getString('date_until')||'').trim(), fee=(interaction.options.getString('fee')||'').trim(), action=(interaction.options.getString('action')||'').trim();
        ...
        return interaction.reply({ content:`✅ ЧС ID: **${row.id}**`, ephemeral:true });
      }
      if (cmd === 'bl-remove') {
        if (!hasRole(interaction.member, BL_ALLOWED_ROLES)) return interaction.reply({ content:'❌ Нет прав.', ephemeral:true });
      ...
      if (cmd === 'bl-find') {
        ...
        if (!rows.length) return interaction.reply({ content:'Не найдено.', ephemeral:true });
...
app.post('/api/register', async (req,res) => {
  try {
    ...
    if (await qOne('SELECT id FROM users WHERE login=$1',[login])) return res.status(400).json({ error:'Логин занят' });
...
app.get('/api/blacklist',authMW,async(_req,res)=>{res.json(await q('SELECT * FROM blacklist ORDER BY id DESC'));});
...
app.post('/api/upload',authMW,upload.single('file'),async(req,res)=>{
  ...
  }catch(e){console.error('upload:',e);res.status(500).json({error:'Ошибка загрузки'});}
});
...
app.get('/api/applications',authMW,async(_req,res)=>{res.json(await q('SELECT * FROM applications ORDER BY id DESC'));});
...
app.post('/api/applications',authMW,async(req,res)=>{
  try{
    ...
    if(bl) return res.status(400).json({error:`Гражданин в ЧС: "${bl.reason}" до ${bl.date_until||'бессрочно'}. Заносил: ${bl.added_by_role}, ${bl.added_by_fio}`,blacklisted:true});
    const uf=extractFIO(req.user.nickname);
...
(async()=>{
  ...
  }catch(e){console.error('❌ Старт:',e);process.exit(1);}
})();
